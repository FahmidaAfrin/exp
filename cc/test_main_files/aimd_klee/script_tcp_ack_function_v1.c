...
int main() 
{
	...
	for (u32 i=0; i<num_events; i++) {
		...
		if ((flag & FLAG_LOST_RETRANS) && (num_loss_events > 0)) { // Loss event
			if (cum_events > 0) {
				/* Process previously cumulative acks */
				update_newly_received_packet_variables (sk, skb, cum_len);
				// Update max_packets_out to not limit snd_cwnd
				tp->max_packets_out = tp->snd_cwnd * 2;
				flag |= FLAG_DATA_ACKED;
				printf("Processing normal event------\n");
			...
			/* Process the loss event*/
			num_loss_events--;
			printf ("Processing loss event------\n");
			...
			// Thus we set this variable to 0 to be able to undo later
			tp->undo_retrans = 0;
			...
			printf ("Undoing the previous loss event------\n");
			...
			__VERIFIER_assert(tp->snd_cwnd >= prior_loss_cwnd);
					...
					|| (flag == 0)) {
			tcp_set_ca_state(sk, TCP_CA_Open);
			/* Aggregate packets to process later*/
			cum_len += len[i];
			cum_events ++;
		}
		else {
