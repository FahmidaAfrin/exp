...
//	u32 num_rtt = 1;
	...
	klee_assume (tp->snd_cwnd >= 1 && tp->snd_cwnd <= 100);
	klee_assume (tp->snd_ssthresh >= 1);
	//jiffies = __VERIFIER_nondet_u32("jiffies");
	//u32 which_flag = __VERIFIER_nondet_u32("which_flag");
	//u32 which_flag[num_events];
	//u32 len[num_events];
	//u32 cum_len = 0;
	u32 agg_update_packets [num_special_events+1];
	u32 one_ack = 1;
	...
	for (u32 i=0; i<num_special_events; i++) {
		...
		agg_update_packets[i] = __VERIFIER_nondet_u32("agg update packets");
		klee_assume (agg_update_packets[i] < tp->snd_cwnd);
		...
		if (three_dup_acks == false) { // Normal case
			...
			update_newly_received_packet_variables (sk, skb, agg_update_packets[i]);
			prior_cwnd = tp->snd_cwnd;
			return_status = aggregate_normal_events (sk, skb);
			// packets acked range from 0 to packets_out
			TCP_SKB_CB(skb)->seq += agg_update_packets[i];
			tp->packets_out -= agg_update_packets[i];
//			klee_assert(tp->snd_cwnd >= prior_cwnd);
		}
		else { // Fast recovery
			printf ("Aggregate in fast recovery------\n");
			update_newly_received_packet_variables_fast_recovery (sk, skb, agg_update_packets[i]);
...
//			klee_assert(tp->snd_cwnd <= prior_loss_cwnd);
		...
		if (undoing[i] != 0 && three_dup_acks == true) { // Cannot undo before the cwnd reduction
			// The undo event
			printf ("Undoing------\n");
			...
			tp->packets_out = __VERIFIER_nondet_u32("packets out after undoing");
			tp->lost_out--;
			tp->retrans_out--;
...
//			klee_assert(tp->snd_cwnd <= prior_loss_cwnd);
		...
		if (i == num_special_events-1) {
			// number of aggregated update packets
			agg_update_packets[num_special_events] = __VERIFIER_nondet_u32("agg last update packets");
			klee_assume (agg_update_packets[num_special_events] < tp->snd_cwnd);
			// Aggregate packets up to one RTT maximum
			if (three_dup_acks == false) {
				printf ("Aggregate last group of update normal packets.\n");
				update_newly_received_packet_variables (sk, skb, agg_update_packets[num_special_events]);
				prior_cwnd = tp->snd_cwnd;
				return_status = aggregate_normal_events (sk, skb);
				TCP_SKB_CB(skb)->seq += agg_update_packets[num_special_events];
				tp->packets_out -= agg_update_packets[num_special_events];
//				klee_assert(tp->snd_cwnd >= prior_cwnd);
			}
			else { // Fast recovery
				printf ("Aggregate last group in fast recovery------\n");
				update_newly_received_packet_variables_fast_recovery (sk, skb, agg_update_packets[num_special_events]);
				// To skip state D case TCP_CA_Recovery in tcp_fastretrans_alert()
				prior_cwnd = tp->snd_cwnd;
				return_status = aggregate_normal_events (sk, skb);
				TCP_SKB_CB(skb)->seq += agg_update_packets[num_special_events];
