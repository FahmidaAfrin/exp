...
int main() 
{
	...
	for (u32 i=0; i<num_special_events; i++) {
		...
		special_indexes[i] = __VERIFIER_nondet_u32("loss event index");
		if (i == 0) { // first loss event
			klee_assume (special_indexes[i] < num_total_events);
			...
			cum_normal_events = special_indexes[i];
...
//			__VERIFIER_assert(tp->snd_cwnd <= prior_loss_cwnd);
		...
		if (i == num_special_events-1) {
			...
			//tcp_set_ca_state(sk, TCP_CA_Open);
			//cum_len = len * (num_events - special_indexes[i] - 1);
			update_newly_received_packet_variables (sk, skb,
								len*(num_total_events - special_indexes[i] - 1));
			// Update max_packets_out to not limit snd_cwnd
			//tp->max_packets_out = tp->snd_cwnd * 2;
			//flag = FLAG_DATA;
			//flag |= FLAG_DATA_ACKED;
			...
			prior_cwnd = tp->snd_cwnd;
			return_status = aggregate_normal_events (sk, skb);
			TCP_SKB_CB(skb)->seq += len*(num_total_events - special_indexes[i] - 1);
//			__VERIFIER_assert(tp->snd_cwnd >= prior_cwnd);
		}
